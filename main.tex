...
\begin{frame}{Reducible Spaces}
    \only<1-3>
    {\pause
    A \textit{reducible} space is one which can be split as a nontrivial Riemannian product. \\
    An \textit{irreducible} space is one which cannot be written as a Riemannian product. \\
    \pause
    Note, Holonomy representation of $M_1 \times M_2$ is $\Hol(M_1) \times \Hol(M_2)$.
    The converse is also true, but locally!\\
    The global converse is achived if we have completeness.
    }
    \only<4>{
...
\begin{frame}{Normal Holonomy Theorem}
    ...
    \begin{theorem}[Normal Holonomy Theorem]
        ...
        on $\nu_p M$, i.e. there exist decompositions
        \begin{align*}
...
\begin{frame}{}
    \begin{theorem}\label{thm:maj}
        ...
        \begin{enumerate}
            \item [(i)] $N^v$ is totally geodesic. And $N^v$ splits off the geodesic $\gamma_v$.
