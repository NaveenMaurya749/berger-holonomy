...
\begin{frame}{Some Results}
    ...
    \end{lemma}
    % Choose any $\xi \in \nu_v(G.v)$ not a multiple of $v$.
    % Change $\xi$ slightly s.t. $\det A_\xi \neq 0$.
    % Let $\gamma(t) = v + t \xi$, and $V = (\m{span}({\nu_{\gamma(t)}(G.\gamma(t))}))\perp$.
\end{frame}
...
\begin{frame}{Full submanifolds}
    ...
    \begin{theorem}
        ...
        Then the connected component of the slice representation $(G_v)_o$ is contained in
        the normal holonomy group $\Hol^\perp_v(G.v)$.
    \end{theorem}
\end{frame}
...
\begin{frame}{Another Lemma}
    We will also use the following lemma eventually:
    \begin{lemma}\label{thm:lem}
        Let $M$ be a Riemannian manifold s.t. every holonomy transformation
        of $T_p M$ extends via the expoenential map tio a local isometry.
...
\begin{frame}{The Glueing Lemma}\label{thm:glue}
    ...
    Proof follows by constructing a parallel frame in which Jacobi operator diagonalizes,
    and then using density.
\end{frame}

\begin{frame}{Auxillary Theorem}
    \begin{theorem}\label{thm:maj}
        ...
        \begin{enumerate}
            ...
            And $Hol_p(N^v) = \Phi^v \subseteq (\Phi_v)_o$.
            \item [(ii)] The normal holonomy group $\Phi^\perp$ of $\Phi.v$ at $v$ 
            acts by isometries on $N^v$. Moreover, $\Phi^\perp \supseteq \Phi^v$, holonomy group of $N^v$ at $p$.
...
\begin{frame}{Auxillary Theorem}
    \begin{enumerate}
        \item [(i)] The proof for totally geodesic follows from the Cartan's criterion for Existence of totally
        geodesic submanifolds, after showing that the curavture endomorphisms
        $R(\xi, \eta)$ belong to the isotropy algebra at $v$, and that the isotropy algebra
        acts invariantly on the normal space $\nu_v(\Phi.v)$. That is, the normal space
        is preserved by the curvature tensor.

        \pause

        \item [(iii)] Let $v \in T_p M$, and $c$ be a curve in $N^v$ from $p$ to $q$.
        Then parallel transport maps $\Phi(p)$ to $\Phi(q)$. \\
        \pause

        So it maps $\Phi.v$ isometrically to $\Phi(q).(P_c(v))$. Then $T_q N^{P_c(v)} = T_qN^v$.
        So $N^{P_c(v)}$ and $N^v$ coincide in a neigbourhood of $q$.
        Thus holonomy transformations of $N^v$ extend to local isometries by the exponential map.
        \pause
        It follows from Lemma \ref{thm:lem}, that $N^v$ is locally symmetric.
    \end{enumerate}
\end{frame}
